...
namespace DTcms.Web.admin.Transport
{
    public partial class goods_list : Web.UI.ManagePage
    {
        ...
        protected string CombSqlTxt(string _keywords)
        {
            ...
            {
                strTemp.Append(" and (Name like '%" + _keywords + "%' or CategoryName like '%" + _keywords + "%' or Code like '%" + _keywords + "%')");
            }
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("goods_list.aspx", "keywords={0}",
                txtKeywords.Text));
        }
        ...
        {
            ChkAdminLevel("goods_list", DTEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.Goods bll = new BLL.Goods();
            ...
            AddAdminLog(DTEnums.ActionEnum.Delete.ToString(), "删除承运货物" + sucCount + "条，失败" + errorCount + "条"); //记录日志
            JscriptMsg("删除成功" + sucCount + "条，失败" + errorCount + "条！",
                Utils.CombUrlTxt("goods_list.aspx", "keywords={0}", this.keywords), "Success");
        }
